/*
 * K-Means Clustering - Versão Paralela com OpenMP
 * 
 * Esta implementação utiliza OpenMP para paralelizar os loops principais
 * do algoritmo K-Means, especificamente as etapas de atribuição de pontos
 * aos clusters e recálculo dos centros.
 * 
 * Estratégia de paralelização:
 * - Contadores locais por thread para evitar condições de corrida
 * - Seções críticas apenas para agregação final dos resultados
 * - Paralelização dos loops de atribuição e reatribuição de clusters
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
double get_precision(int *x, int *y, const int n) {
    // x = ideal (classes reais), y = resultado (clusters encontrados)
    
    // 1. Descobrir o número máximo de classes/clusters para alocar a matriz
    int max_x = 0, max_y = 0;
    for (int i = 0; i < n; ++i) {
    ...
    // 2. Alocar Matriz de Contingência (Confusion Matrix)
    // counts[i][j] = quantos pontos da classe 'i' cairam no cluster 'j'
    ...
    // 3. Preencher a matriz (Passada única O(N))
    for (int i = 0; i < n; ++i) {
        counts[x[i]][y[i]]++;
    }

    // 4. Calcular yy e (yy+ny) usando combinatória
    ...
    // Para cada cluster encontrado (coluna j)
    for (int j = 0; j < cols; ++j) {
        unsigned long total_in_cluster = 0;
        
        // Para cada classe real (linha i)
        for (int i = 0; i < rows; ++i) {
            unsigned long count = counts[i][j];
            total_in_cluster += count;
            
            // Combinação de 2 em 2: count * (count - 1) / 2
            if (count > 1) {
                yy += (count * (count - 1)) / 2;
            }
        }
 ...
 * Paralelização: cada thread mantém contadores locais, agregados ao final
 */
                     ...
                     const int n, const int m, const int k)
{
    ...
    /* Paraleliza reatribuição de pontos aos clusters com novos centros */
    #pragma omp parallel
    {
        int changed_local = 0;
        int *nums_local = (int*)calloc(k, sizeof(int));
...
int* kmeans(const double* const X, const int n, const int m, const int k) {
    ...
    while (1) {
        char changed = check_partition(x, c, y, nums, n, m, k);

        if (!changed) {
            printf("[KMEANS] Convergiu na iteracao %d\n", iter);
            break;
        }
